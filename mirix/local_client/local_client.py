...
class LocalClient(AbstractClient):
    ...
    def list_agents(
        ...
        return self.server.agent_manager._sync_list_agents(
            actor=self.client,
            tags=tags,
    ...
    def create_agent(
        ...
        # TODO: get full agent state (sync: LocalClient is sync)
        return self.server.agent_manager._sync_get_agent_by_id(agent_state.id, actor=self.client)
    ...
    def get_tools_from_agent(self, agent_id: str) -> List[Tool]:
        ...
        """
        self.interface.clear()
        return self.server.agent_manager._sync_get_agent_by_id(
            agent_id=agent_id,
            actor=self.client,
        ).tools
    ...
    def get_in_context_messages(self, agent_id: str, user_id: Optional[str] = None) -> List[Message]:
        ...
        """
        agent_state = self.server.agent_manager._sync_get_agent_by_id(
            agent_id=agent_id,
            actor=self.client,
        )
        user = None
    ...
    def retrieve_memory(
        ...
        # Get the agent to access its memory managers (sync: LocalClient is sync)
        agent_state = self.server.agent_manager._sync_get_agent_by_id(
            agent_id=agent_id,
            actor=self.client,
        )
