...
def resource_memory_insert(self: "Agent", items: List[ResourceMemoryItemBase]):
    ...
    for item in items:
        ...
        # Use sync wrapper: tool runs in sync context (agent step)
        existing_resources = self.resource_memory_manager._sync_list_resources(
            agent_state=self.agent_state,
...
def procedural_memory_insert(self: "Agent", items: List[ProceduralMemoryItemBase]):
    ...
    for item in items:
        ...
        # Use sync wrapper: tool runs in sync context (agent step)
        existing_procedures = self.procedural_memory_manager._sync_list_procedures(
            agent_state=self.agent_state,
            user=self.user,  # User for read operations (data filtering)
...
def semantic_memory_insert(self: "Agent", items: List[SemanticMemoryItemBase]):
    ...
    for item in items:
        ...
        # Use sync wrapper: tool runs in sync context (agent step)
        existing_items = self.semantic_memory_manager._sync_list_semantic_items(
            agent_state=self.agent_state,
            user=self.user,  # User for read operations (data filtering)
            query="",  # Get all items
            limit=1000,  # Get enough to check for duplicates
...
def knowledge_vault_insert(self: "Agent", items: List[KnowledgeVaultItemBase]):
    ...
    for item in items:
        ...
        # Use sync wrapper: tool runs in sync context (agent step)
        existing_items = self.knowledge_vault_manager._sync_list_knowledge(
            agent_state=self.agent_state,
            user=self.user,  # User for read operations (data filtering)
            query="",  # Get all items
            limit=1000,  # Get enough to check for duplicates
