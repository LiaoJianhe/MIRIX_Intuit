...
class BlockManager:
            ...
            if cache_provider:
                ...
                if cached_data:
                    if "value" not in cached_data or cached_data["value"] is None:
                        cached_data["value"] = ""
                    # Scoped core memory (PR 54): ensure filter_tags is dict when from cache
                    if "filter_tags" in cached_data and isinstance(
                        cached_data["filter_tags"], str
                    ):
                        import json as _json
                        try:
                            cached_data["filter_tags"] = (
                                _json.loads(cached_data["filter_tags"])
                                if cached_data["filter_tags"]
                                else None
                            )
                        except (TypeError, ValueError):
                            cached_data["filter_tags"] = None
    ...
    async def soft_delete_by_user_id(self, user_id: str) -> int:
        ...
        def _db_soft_delete():
            with self.session_maker() as session:
                blocks = (
                    ...
                    .filter(BlockModel.user_id == user_id, BlockModel.is_deleted == False)
                    .all()
                )
                count = len(blocks)
                if count == 0:
        ...
        count, block_ids = await asyncio.to_thread(_db_soft_delete)
        if count == 0:
            return 0
    ...
    async def delete_by_user_id(self, user_id: str) -> int:
        ...
        def _db_delete():
            with self.session_maker() as session:
                block_ids = [
                    row[0] for row in session.query(BlockModel.id).filter(BlockModel.user_id == user_id).all()
                ]
                count = len(block_ids)
                if count == 0:
                    return 0, []
