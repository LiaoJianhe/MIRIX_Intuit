...
class EpisodicMemoryManager:
    ...
    async def get_episodic_memory_by_id(
        ...
        async with self.session_maker() as session:
            try:
                ...
                episodic_memory_item = await EpisodicEvent.read(
    ...
    @update_timezone
    @enforce_types
    async def get_most_recently_updated_event(
        ...
        if provider:
            ...
            return PydanticEpisodicEvent(**row) if row else None
    ...
    async def create_episodic_memory(
        ...
        # PostgreSQL episodic_memory.occurred_at is TIMESTAMP WITHOUT TIME ZONE;
        # asyncpg fails if we pass a timezone-aware datetime. Normalize to naive UTC.
        occ = episodic_memory_dict.get("occurred_at")
        if isinstance(occ, datetime) and occ.tzinfo is not None:
            episodic_memory_dict["occurred_at"] = occ.astimezone(timezone.utc).replace(tzinfo=None)

        # Other fields like created_at, etc. may be auto-generated by the model or the DB

        # Create the episodic memory item (with conditional Redis caching)
        async with self.session_maker() as session:
            episodic_memory_item = EpisodicEvent(**episodic_memory_dict)
            await episodic_memory_item.create_with_redis(session, actor=actor, use_cache=use_cache)
            return episodic_memory_item.to_pydantic()

    @enforce_types
    async def create_many_episodic_memory(
        self,
        ...
        """
        return [await self.create_episodic_memory(e, actor, client_id, user_id) for e in episodic_memory]

    @enforce_types
    async def delete_event_by_id(self, id: str, actor: PydanticClient) -> None:
        """
        ...
        provider = get_relational_provider()
        if provider:
            await provider.delete("episodic_memory", id, actor=actor)
            try:
        ...
        async with self.session_maker() as session:
            try:
                ...
                await episodic_memory_item.hard_delete(session)
            except NoResultFound:
    ...
    async def soft_delete_by_client_id(self, actor: PydanticClient) -> int:
        ...
        # Invalidate Redis cache (episodic keys are JSON type, not hash; delete to avoid WRONGTYPE)
        redis_client = get_redis_client()
        if redis_client:
    ...
    async def soft_delete_by_user_id(self, user_id: str) -> int:
        ...
        # Invalidate Redis cache in batch (episodic keys are JSON type, not hash; delete to avoid WRONGTYPE)
        redis_client = get_redis_client()
        if redis_client and item_ids:
            try:
    ...
    @update_timezone
    @enforce_types
    async def list_episodic_memory_around_timestamp(
        self,
        agent_state: AgentState,
        start_time: datetime,
        end_time: datetime,
        ...
        async with self.session_maker() as session:
            ...
            if query == "":
                query_stmt = (
                    ...
                    .where(~EpisodicEvent.is_deleted)
    ...
    async def update_event(
        ...
        provider = get_relational_provider()
        if provider:
            existing = await provider.read("episodic_memory", event_id, actor=actor)
            if not existing:
            ...
            try:
                ...
                await invalidate_memory_cache("episodic_memory", [event_id])
            except Exception as exc:
                logger.warning(
                    "Cache invalidation skipped for episodic_memory %s: %s",
                    event_id,
                    exc,
                )
            return PydanticEpisodicEvent(**result)
