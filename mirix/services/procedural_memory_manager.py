...
class ProceduralMemoryManager:
    ...
    def update_item(
        ...
        with self.session_maker() as session:
            item = ProceduralMemoryItem.read(db_session=session, identifier=item_update.id, user=user)
    ...
    async def list_procedures(
        ...
        if use_cache and search_provider:
            try:
                ...
                if is_empty_query:
                    logger.debug("Searching for recent procedural items with filter_tags=%s", filter_tags)
                    results = await search_provider.search_recent(
                        index_name=search_provider.PROCEDURAL_INDEX,
                        limit=limit or 50,
                        user_id=user.id,
                        organization_id=organization_id,
                        filter_tags=filter_tags,
                    )
                    logger.debug("search_recent returned %d results", len(results) if results else 0)
                    if results:
                ...
                # Case 2: Vector similarity search
                    ...
                    if results:
                        logger.debug("Vector search HIT: found %d procedural items", len(results))
                        results = search_provider.clean_search_fields(results)
                        return [PydanticProceduralMemoryItem(**item) for item in results]
                    ...
                    if results:
                        logger.debug("Text search HIT: found %d procedural items", len(results))
                        results = search_provider.clean_search_fields(results)
                        return [PydanticProceduralMemoryItem(**item) for item in results]

            except Exception as e:
                logger.warning("Search failed for procedural memory, falling back to PostgreSQL: %s", e)
        ...
        if not use_cache:
            ...
            logger.debug("No search provider, querying PostgreSQL directly for procedural memory")
    ...
    async def delete_by_client_id(self, actor: PydanticClient) -> int:
        ...
        def _db_delete():
            with self.session_maker() as session:
                item_ids = [
                    row[0]
                    for row in session.query(ProceduralMemoryItem.id)
                    .filter(ProceduralMemoryItem.client_id == actor.id)
                    .all()
                ]
                count = len(item_ids)
                if count == 0:
                    return 0, []
                session.query(ProceduralMemoryItem).filter(ProceduralMemoryItem.client_id == actor.id).delete(
        ...
        if cache_provider and item_ids:
            redis_keys = [f"{cache_provider.PROCEDURAL_PREFIX}{item_id}" for item_id in item_ids]
            BATCH_SIZE = 1000
    ...
    async def soft_delete_by_client_id(self, actor: PydanticClient) -> int:
        ...
        def _db_soft_delete():
            with self.session_maker() as session:
                items = (
                    session.query(ProceduralMemoryItem)
                    .filter(ProceduralMemoryItem.client_id == actor.id, ProceduralMemoryItem.is_deleted == False)
                    .all()
                )
                count = len(items)
                if count == 0:
        ...
        if cache_provider:
            for item_id in item_ids:
                redis_key = f"{cache_provider.PROCEDURAL_PREFIX}{item_id}"
    ...
    async def soft_delete_by_user_id(self, user_id: str) -> int:
        ...
        def _db_soft_delete():
            with self.session_maker() as session:
                items = (
                    session.query(ProceduralMemoryItem)
                    .filter(ProceduralMemoryItem.user_id == user_id, ProceduralMemoryItem.is_deleted == False)
                    .all()
                )
                count = len(items)
                if count == 0:
    ...
    async def delete_by_user_id(self, user_id: str) -> int:
        ...
        def _db_delete():
            with self.session_maker() as session:
                item_ids = [
                    row[0]
                    for row in session.query(ProceduralMemoryItem.id).filter(
                ...
                if count == 0:
                    return 0, []
                session.query(ProceduralMemoryItem).filter(ProceduralMemoryItem.user_id == user_id).delete(
                    synchronize_session=False
                )
                session.commit()
                return count, item_ids
    ...
    @update_timezone
    @enforce_types
    async def list_procedures_by_org(
        self,
        agent_state: AgentState,
        organization_id: str,
        query: str = "",
        ...
        if use_cache and search_provider:
            try:
                if not query or query == "":
                    ...
                    if results:
                        logger.debug("Vector: %d results", len(results))
                        results = search_provider.clean_search_fields(results)
                        return [PydanticProceduralMemoryItem(**item) for item in results]
                    ...
                    if results:
                        logger.debug("Text: %d results", len(results))
                        results = search_provider.clean_search_fields(results)
                        return [PydanticProceduralMemoryItem(**item) for item in results]
            except Exception as e:
                ...
                for key, value in filter_tags.items():
                    if key == "read_scopes":
                        ...
                        if isinstance(value, list) and value:
                            scope_conditions = [
                                ProceduralMemoryItem.filter_tags["scope"].as_string() == scope for scope in value
                            ]
