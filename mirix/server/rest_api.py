...
def create_langfuse_tracing_middleware(
    traced_patterns: Optional[List[str]] = None,
    trace_all: bool = True,
) -> Callable:
    """
    Create a Langfuse tracing middleware with configurable endpoint filtering.

    This factory function returns a middleware dispatch function that can be used
    with FastAPI's BaseHTTPMiddleware or the @app.middleware("http") decorator.

    Args:
        traced_patterns: List of regex patterns for endpoints to trace.
                        Pattern format: "METHOD /path" (e.g., "POST /agents/.+/messages").
                        Only used when trace_all=False.
        trace_all: If True (default), traces all endpoints.
                  If False, only traces endpoints matching traced_patterns.

    Returns:
        Middleware dispatch function for use with BaseHTTPMiddleware

    Examples:
        # Default: trace ALL endpoints
        app.add_middleware(
            BaseHTTPMiddleware,
            dispatch=create_langfuse_tracing_middleware()
        )

        # Custom: only trace specific patterns
        app.add_middleware(
            BaseHTTPMiddleware,
            dispatch=create_langfuse_tracing_middleware(
                traced_patterns=[
                    r"^POST /my-custom-agent-route$",
                    r"^POST /chat/.*$",
                ],
                trace_all=False
            )
    ...
    """
    # Compile patterns once at middleware creation time for efficiency
    compiled_patterns = []
    if traced_patterns:
        compiled_patterns = [re.compile(pattern) for pattern in traced_patterns]

    async def middleware(request: Request, call_next):
        """
        LangFuse tracing middleware dispatch function.

        This middleware:
        1. Checks if the endpoint should be traced (based on configuration)
        2. Creates a root span for traced requests
        ...
        endpoint_signature = f"{method} {path}"

        # Check if this endpoint should be traced
        if not trace_all:
            if not compiled_patterns:
                # No patterns specified and trace_all=False means skip all tracing
                return await call_next(request)

            # Check if endpoint matches any pattern
            should_trace = any(
                pattern.match(endpoint_signature) for pattern in compiled_patterns
            )
            if not should_trace:
                return await call_next(request)
...
# Traces all endpoints (original behavior)
langfuse_tracing_middleware = create_langfuse_tracing_middleware()


# Register the default middleware with the app
@app.middleware("http")
async def _langfuse_tracing_middleware(request: Request, call_next):
    """Wrapper to register the default Langfuse tracing middleware with the app."""
    return await langfuse_tracing_middleware(request, call_next)
