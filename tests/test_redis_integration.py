...
async def redis_client(_ensure_tables_and_redis):
    ...
    # Ensure global client exists for managers (cache provider)
    ...
    # Use a new client bound to this event loop for test verification (read/write in test body)
...
@pytest_asyncio.fixture
async def test_organization(organization_manager):
    """Create and persist a test organization."""
    org = PydanticOrganization(id=generate_test_id("org"), name="Test Organization")
...
@pytest_asyncio.fixture
async def test_user(test_organization, user_manager):
    ...
    # Cleanup
    try:
        await user_manager.delete_user_by_id(created_user.id)
    except Exception:  # pylint: disable=broad-except
        pass  # May already be deleted


@pytest_asyncio.fixture
async def test_client(test_organization):
...
@pytest_asyncio.fixture
async def test_agent(test_client, agent_manager):
...
class TestOrganizationManagerRedis:
    ...
    async def test_organization_create_with_redis(self, organization_manager, redis_client):
        """Test creating an organization caches to Redis Hash."""
        org_data = PydanticOrganization(id=generate_test_id("org"), name="Test Corp")

        # Create organization (async)
        created_org = await organization_manager.create_organization(org_data)

        # Verify in Redis Hash (native async client)
        redis_key = f"{redis_client.ORGANIZATION_PREFIX}{created_org.id}"
        cached_data = await redis_client.get_hash(redis_key)
    ...
    async def test_organization_cache_hit(self, organization_manager, redis_client):
        """Test cache hit for organization retrieval."""
        # Create organization (async)
        org_data = PydanticOrganization(id=generate_test_id("org"), name="Cache Test Org")
        created_org = await organization_manager.create_organization(org_data)

        # First get (should populate cache)
        org1 = await organization_manager.get_organization_by_id(created_org.id)

        # Second get (should hit cache)
        start_time = time.time()
        org2 = await organization_manager.get_organization_by_id(created_org.id)
    ...
    async def test_organization_update_invalidates_cache(self, organization_manager, redis_client):
        """Test updating organization updates Redis cache."""
        # Create organization (async)
        org_data = PydanticOrganization(id=generate_test_id("org"), name="Original Name")
        created_org = await organization_manager.create_organization(org_data)

        # Update organization
        await organization_manager.update_organization_name_using_id(created_org.id, name="Updated Name")

        # Verify Redis cache is updated (native async client)
        redis_key = f"{redis_client.ORGANIZATION_PREFIX}{created_org.id}"
        cached_data = await redis_client.get_hash(redis_key)
    ...
    async def test_organization_delete_removes_cache(self, organization_manager, redis_client):
        """Test deleting organization removes from Redis cache."""
        # Create organization (async)
        org_data = PydanticOrganization(id=generate_test_id("org"), name="To Be Deleted")
        created_org = await organization_manager.create_organization(org_data)

        # Verify in cache (native async client)
        redis_key = f"{redis_client.ORGANIZATION_PREFIX}{created_org.id}"
        assert await redis_client.get_hash(redis_key) is not None

        # Delete organization
        await organization_manager.delete_organization_by_id(created_org.id)

        # Verify removed from cache
        assert await redis_client.get_hash(redis_key) is None
...
class TestUserManagerRedis:
    ...
    async def test_user_create_with_redis(self, test_user, redis_client):
    ...
    async def test_user_cache_hit_performance(self, user_manager, test_user, redis_client):
    ...
    async def test_user_update_status_invalidates_cache(self, user_manager, test_user, redis_client):
        """Test updating user status updates Redis cache."""
        # test_user fixture already creates the user
        # Update status
        await user_manager.update_user_status(test_user.id, "inactive")
    ...
    async def test_user_update_timezone_invalidates_cache(self, user_manager, test_user, redis_client):
        """Test updating user timezone updates Redis cache."""
        # test_user fixture already creates the user
        # Update timezone
        await user_manager.update_user_timezone("Europe/London", test_user.id)
    ...
    async def test_user_delete_removes_cache(self, user_manager, test_user, redis_client):
        ...
        await user_manager.delete_user_by_id(test_user.id)

        # Verify cache is invalidated (key removed) so soft-deleted user is not served from cache
        cached_data = await redis_client.get_hash(redis_key)
        assert cached_data is None, "Soft-deleted user should be removed from cache (invalidated)"
...
class TestAgentAndToolManagerRedis:
    ...
    async def test_agent_create_with_tools_caches_both(self, agent_manager, test_client, redis_client):
    ...
    async def test_agent_with_tools_denormalizes_tools_agents(self, agent_manager, test_client, redis_client):
    ...
    async def test_agent_retrieval_uses_pipeline_for_tools(self, agent_manager, test_client, redis_client):
        ...
        start_time = time.time()
        retrieved_agent = await agent_manager.get_agent_by_id(created_agent.id, test_client)
    ...
    async def test_agent_update_invalidates_cache(self, agent_manager, test_client, redis_client):
        ...
        await agent_manager.update_agent(created_agent.id, update_data, test_client)
    ...
    async def test_agent_delete_removes_cache(self, agent_manager, test_client, redis_client):
    ...
    async def test_agent_cache_hit_performance(self, agent_manager, test_client, redis_client):
        ...
        # Warm cache
        await agent_manager.get_agent_by_id(created_agent.id, test_client)
...
class TestToolsAgentsDenormalization:
    ...
    async def test_tools_agents_not_cached_separately(self, agent_manager, test_client, redis_client):
        ...
        junction_keys = await redis_client.client.keys("tools_agents:*")
    ...
    async def test_agent_with_tools_single_pipeline_retrieval(self, agent_manager, test_client, redis_client):
        ...
        start_time = time.time()
        agent = await agent_manager.get_agent_by_id(created_agent.id, test_client)
...
class TestBlockManagerRedis:
    ...
    async def test_block_create_with_redis(self, block_manager, test_client, test_user, redis_client):
        """Test creating a block caches to Redis Hash."""
        block_data = PydanticBlock(
            id=generate_test_id("block"), label="persona", value="I am a test assistant", limit=2000
        )

        # Create block
        created_block = await block_manager.create_or_update_block(block_data, actor=test_client, user=test_user)
        ...
        # Cleanup
        await block_manager.delete_block(created_block.id, test_client)

    async def test_block_cache_hit(self, block_manager, test_client, test_user, redis_client):
        """Test cache hit for block retrieval."""
        # Create block
        block_data = PydanticBlock(id=generate_test_id("block"), label="human", value="Test user info", limit=2000)
        created_block = await block_manager.create_or_update_block(block_data, test_client, user=test_user)

        # First get (should populate cache)
        block1 = await block_manager.get_block_by_id(created_block.id, test_user)

        # Second get (should hit cache)
        start_time = time.time()
        block2 = await block_manager.get_block_by_id(created_block.id, test_user)
        cache_time = time.time() - start_time
    ...
    async def test_block_update_invalidates_cache(self, block_manager, test_client, test_user, redis_client):
        """Test updating a block updates Redis cache."""
        # Create block
        block_data = PydanticBlock(id=generate_test_id("block"), label="system", value="Original value", limit=2000)
        created_block = await block_manager.create_or_update_block(block_data, test_client, user=test_user)

        # Update block
        update_data = BlockUpdate(value="Updated value")
        await block_manager.update_block(created_block.id, update_data, test_client, user=test_user)
        ...
        retrieved_block = await block_manager.get_block_by_id(created_block.id, test_user)
        assert retrieved_block.value == "Updated value"

        # Cleanup
        await block_manager.delete_block(created_block.id, test_client)

    async def test_block_delete_removes_cache(self, block_manager, test_client, test_user, redis_client):
        """Test deleting a block removes from Redis cache."""
        # Create block
        block_data = PydanticBlock(id=generate_test_id("block"), label="persona", value="To be deleted", limit=2000)
        created_block = await block_manager.create_or_update_block(block_data, test_client, user=test_user)
...
class TestMessageManagerRedis:
    ...
    async def test_message_create_with_redis(self, message_manager, test_client, test_user, test_agent, redis_client):
...
class TestEpisodicMemoryManagerRedis:
    ...
    async def test_episodic_create_with_redis(self, episodic_manager, test_client, test_user, test_agent, redis_client):
        ...
        # Create event (async)
        created_event = await episodic_manager.create_episodic_memory(event_data, test_client, user_id=test_user.id)
...
class TestSemanticMemoryManagerRedis:
    ...
    async def test_semantic_create_with_three_embeddings(
        self, semantic_manager, test_client, test_user, test_agent, redis_client
    ):
        ...
        # Create item
        created_item = await semantic_manager.create_item(item_data, test_client, user_id=test_user.id)
        ...
        # Cleanup
        await semantic_manager.delete_semantic_item_by_id(created_item.id, test_client)
...
class TestProceduralMemoryManagerRedis:
    ...
    async def test_procedural_create_with_embeddings(
        self, procedural_manager, test_client, test_user, test_agent, redis_client
    ):
        ...
        # Create item
        created_item = await procedural_manager.create_item(item_data, test_client, user_id=test_user.id)
        ...
        # Cleanup
        await procedural_manager.delete_procedure_by_id(created_item.id, test_client)
...
class TestResourceMemoryManagerRedis:
    ...
    async def test_resource_create_with_embedding(
        ...
        # Create item
        created_item = await resource_manager.create_item(item_data, test_client, user_id=test_user.id)
        ...
        # Cleanup
        await resource_manager.delete_resource_by_id(created_item.id, test_client)
...
class TestKnowledgeVaultManagerRedis:
    ...
    async def test_knowledge_create_with_embedding(
        ...
        # Create item
        created_item = await knowledge_manager.create_item(item_data, test_client, user_id=test_user.id)
        ...
        # Cleanup
        await knowledge_manager.delete_knowledge_by_id(created_item.id, test_client)
...
class TestRedisFallback:
    ...
    async def test_block_manager_works_without_redis(self, block_manager, test_client, test_user):
        ...
        try:
            ...
            block_data = PydanticBlock(id=generate_test_id("block"), label="persona", value="No Redis test", limit=2000)
...
class TestRedisPerformance:
    ...
    async def test_block_cache_speedup(self, block_manager, test_client, test_user, redis_client):
        """Measure speedup from Redis Hash caching."""
        # Create test blocks
        blocks = []
        for i in range(10):
            block_data = PydanticBlock(
                id=generate_test_id("block"), label=f"test-{i}", value=f"Test value {i}", limit=2000
            )
            created_block = await block_manager.create_or_update_block(block_data, test_client, user=test_user)
        ...
        # Warm up cache
        for block in blocks:
            await block_manager.get_block_by_id(block.id, test_user)
...
class TestRedisIntegrationEnd2End:
    ...
    async def test_full_workflow_with_redis(
        ...
        # 1. Create block (Hash) (async)
        block = PydanticBlock(
            id=generate_test_id("block"), label="persona", value="I am a helpful assistant", limit=2000
        )
        created_block = await block_manager.create_or_update_block(block, test_client, user=test_user)

        # 2. Create message (Hash) (async)
        ...
        created_event = await episodic_manager.create_episodic_memory(event, test_client, user_id=test_user.id)

        # 4. Create semantic memory (JSON with 3 embeddings) (async)
        ...
        created_semantic = await semantic_manager.create_item(semantic, test_client, user_id=test_user.id)

        # 5. Verify all cached
        assert await redis_client.get_hash(f"{redis_client.BLOCK_PREFIX}{created_block.id}") is not None
        assert await redis_client.get_hash(f"{redis_client.MESSAGE_PREFIX}{created_message.id}") is not None
        assert await redis_client.get_json(f"{redis_client.EPISODIC_PREFIX}{created_event.id}") is not None
        assert await redis_client.get_json(f"{redis_client.SEMANTIC_PREFIX}{created_semantic.id}") is not None

        # 6. Retrieve all (should hit cache) (async)
        retrieved_block = await block_manager.get_block_by_id(created_block.id, test_user)
        retrieved_message = await message_manager.get_message_by_id(created_message.id, test_client)
        retrieved_event = await episodic_manager.get_episodic_memory_by_id(created_event.id, test_user)
        retrieved_semantic = await semantic_manager.get_semantic_item_by_id(
            created_semantic.id, test_user, test_user.timezone
        )
        ...
        await message_manager.delete_message_by_id(created_message.id, test_client)
        await episodic_manager.delete_event_by_id(created_event.id, test_client)
        await semantic_manager.delete_semantic_item_by_id(created_semantic.id, test_client)

        # Verify all removed from cache
        assert await redis_client.get_hash(f"{redis_client.BLOCK_PREFIX}{created_block.id}") is None
        assert await redis_client.get_hash(f"{redis_client.MESSAGE_PREFIX}{created_message.id}") is None
        assert await redis_client.get_json(f"{redis_client.EPISODIC_PREFIX}{created_event.id}") is None
        assert await redis_client.get_json(f"{redis_client.SEMANTIC_PREFIX}{created_semantic.id}") is None
...
async def test_redis_integration_summary(redis_client):  # pylint: disable=unused-argument
