...
def event_loop_sync_bridge():
    """Provide an event loop for the sync_bridge so _sync_get_agent_by_id and create_agent work."""
...
class _RedisSyncWrapper:
    """Sync wrapper so tests can call get_json() from sync context on the same loop as the client."""

    def __init__(self, client, loop):
        self._client = client
        self._loop = loop
        self.RAW_MEMORY_PREFIX = getattr(
            client, "RAW_MEMORY_PREFIX", "raw_memory:"
        )

    def get_json(self, key: str):
        return asyncio.run_coroutine_threadsafe(
            self._client.get_json(key), self._loop
        ).result(timeout=5)


@pytest.fixture
def redis_client(event_loop_sync_bridge):
    """Initialize Redis client for testing (uses sync_bridge loop so cache provider and tests share one loop)."""
    ...
    loop = event_loop_sync_bridge
    client = get_redis_client()
    if client is None:
        client = asyncio.run_coroutine_threadsafe(
            initialize_redis_client(), loop
        ).result(timeout=15)
...
def test_api_update_raw_memory_replace(
    ...
    if not (
        ...
        or os.getenv("MIRIX_GOOGLE_API_KEY")
    ):
        pytest.skip("Skipping API test with embeddings - no Gemini/Google API key (GEMINI_API_KEY, GOOGLE_API_KEY, or MIRIX_GOOGLE_API_KEY)")
...
@pytest.fixture(scope="module")
def test_agent(event_loop_sync_bridge, test_actor):
    ...
    try:
        return agent_mgr._sync_get_agent_by_id(agent_id=agent_id, actor=test_actor)
    except Exception:
...
def test_search_raw_memories_filter_tags_multiple_keys(raw_memory_manager, test_actor, test_user):
    ...
    mem1 = raw_memory_manager.create_raw_memory(
        raw_memory=RawMemoryItemCreate(
            context="Memory 1",
            filter_tags={"scope": test_actor.write_scope, "priority": "high", "source": "iep"},
            user_id=test_user.id,
            organization_id=test_actor.organization_id,
            occurred_at=None,
            id=None,
    ...
    mem2 = raw_memory_manager.create_raw_memory(
        raw_memory=RawMemoryItemCreate(
            context="Memory 2",
            filter_tags={"scope": test_actor.write_scope, "priority": "high", "source": "manual"},
            user_id=test_user.id,
            organization_id=test_actor.organization_id,
            occurred_at=None,
            id=None,
    ...
    mem3 = raw_memory_manager.create_raw_memory(
        raw_memory=RawMemoryItemCreate(
            context="Memory 3",
            filter_tags={"scope": test_actor.write_scope, "priority": "low", "source": "iep"},
            user_id=test_user.id,
            organization_id=test_actor.organization_id,
            occurred_at=None,
            id=None,
...
def test_search_raw_memories_filters_by_user_id(raw_memory_manager, test_actor, test_user, test_user_different):
    ...
    mem1 = raw_memory_manager.create_raw_memory(
        raw_memory=RawMemoryItemCreate(
            ...
            filter_tags={"scope": test_actor.write_scope, "test_tag": "user_filter_test"},
            user_id=test_user.id,
            organization_id=test_actor.organization_id,
        ),
        actor=test_actor,
