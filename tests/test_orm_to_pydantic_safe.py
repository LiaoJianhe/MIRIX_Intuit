...
async def test_episodic_memory_to_pydantic(server):
    ...
    async with db_context() as session:
        result = await session.execute(select(EpisodicEvent).where(EpisodicEvent.id == event.id))
...
async def test_memory_models_to_pydantic(server):
    ...
    async with db_context() as session:
        result = await session.execute(select(SemanticMemoryItem).where(SemanticMemoryItem.id == semantic.id))
    ...
    async with db_context() as session:
        result = await session.execute(select(ProceduralMemoryItem).where(ProceduralMemoryItem.id == procedural.id))
    ...
    async with db_context() as session:
        result = await session.execute(select(ResourceMemoryItem).where(ResourceMemoryItem.id == resource.id))
    ...
    async with db_context() as session:
        result = await session.execute(select(KnowledgeVaultItem).where(KnowledgeVaultItem.id == knowledge.id))
