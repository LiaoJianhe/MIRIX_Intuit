...
ECMS imports MIRIX route handlers and calls them directly (not through FastAPI's
HTTP layer), so the ``Header(None)`` defaults are never resolved. A caller that
omits ``x_org_id`` passes the ``Header(None)`` FieldInfo sentinel — which is
truthy — so ``x_org_id or DEFAULT_ORG_ID`` would yield the sentinel instead of
the default org, later failing @enforce_types deep in the call (500 on save).

get_client_and_org normalizes any non-str sentinel back to None so the
default-org fallback fires. Regression for VEPAGE-1155 full-stack save 500s.

Run:
    pytest tests/test_get_client_and_org_direct_call.py -v
"""
...
@pytest.mark.asyncio
async def test_client_id_header_sentinel_is_normalized_to_required_error():
    # If BOTH client id and api key are unresolved sentinels, the function must
    # treat them as absent and raise the 401 (not silently use a FieldInfo).
